<template>
  <div class="dropdown">
    <button class="dropdown__btn btn" :class="[{'dropdown__btn_disabled': !drop}, `dropdown__btn_color-border-${colorType}`]" @click="open = !open">
      ...
      {{ oldKey }}
      <div v-if="methodName && !!scheme && scheme[`${oldKey}`]" class="dropdown__text">
        {{ scheme[`${oldKey}`] }} 1
      </div>
    </button>
    ...
    <template v-if="open && typeof json === 'object' && !Array.isArray(json)">
      <BaseDropdown
        ...
        :json="item ? item : {}"
        :title="`${key}`"
        :key-title="funcKeyTitle(keyTitle, key)"
...
export default {
  ...
  computed: {
    oldKey () {
      return `${this.keyTitle || this.title}`.toLowerCase()
    },
    scheme () {
      if (this.methodName) {
  ...
  methods: {
    ...
    funcKeyTitle  (keyTitle, key) {
      const value = this.keyTitle ? `${this.keyTitle}_${key}` : key
