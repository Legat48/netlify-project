...
<template>
  <div v-if="!$store.getters.getLoading" class="content">
    <StatusWrap :method-name="methodName" class="content__wrap" />
  </div>
...
export default {
  asyncData ({ store }) {
    store.commit('header/setHeaderTitle', ' JSON статус оборудования')
  },
  data () {
    return {
      pageTitle: 'ОДС | JSON статус оборудования',
